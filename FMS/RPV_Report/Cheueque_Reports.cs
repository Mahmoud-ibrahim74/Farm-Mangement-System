...
{
    public partial class Cheueque_Reports : Form
    {
        DataTable dt = new DataTable();
        DataTable dt3 = new DataTable();


        public Cheueque_Reports()
        {
            InitializeComponent();
...
" *- Press (Merge Button)  to merge two or more PDF files in one PDF file.");
        ...
        private void guna2Button3_Click(object sender, System.EventArgs e)
        {
            ...
            sound.RunClickSound();
            if (getBanck.SelectedIndex == -1 || service_type.SelectedIndex == -1)
            {
        ...
        private void backgroundWorker_DoWork(object sender, System.ComponentModel.DoWorkEventArgs e)
        {
            ...
            SQLSERVER_Queries.RPV_Report_Scripts.chequeReportsScripts cheque = new SQLSERVER_Queries.RPV_Report_Scripts.chequeReportsScripts();
            ...
            if (ControlName == "displayData")
            {
                dt.Clear();
                dt3.Clear();
                cheque.DisplayData(service_type, getBanck, dt, dt3);
            ...
            else if (ControlName == "displayByInterval")
            {
                dt.Clear();
                dt3.Clear();
                cheque.DisplayByInterval(service_type, getBanck, fromDate, toDate, dt, dt3);
                tableB.DataSource = dt;
                tableA.DataSource = dt3;
            }
            else
            {
                cheque.LoadData(getBanck);
            }
        ...
        private void guna2GradientButton1_Click(object sender, EventArgs e)
        {
            ...
            else
            {
                ...
                if (service_type.SelectedIndex == 0)
                {
                    ...
                    calc.Columns.Add("afterAdded", "رصيد البنك");
                    for (int i = 0; i < tableB.Rows.Count; i++)
                    {
                        getTotal = getTotal + Convert.ToDouble(tableB[3, i].Value.ToString());

                    }
                    getTotal_Bank = Convert.ToDouble(tableA[3, 0].Value.ToString());    // get balance of Tresuary

                    calc.Rows.Add(getTotal, getTotal_Bank);
                ...
                else if (service_type.SelectedIndex == 1)
                {
                    calc.Rows.Clear();
                    calc.Columns.Clear();
                    calc.Columns.Add("total", "إجمالي السحب");
                    calc.Columns.Add("afterAdded", "رصيد البنك بعد السحب");
                    for (int i = 0; i < tableB.Rows.Count; i++)
                    {
                        getTotal = getTotal + Convert.ToDouble(tableB[3, i].Value.ToString());
                ...
                else
                {
                    ...
                    calc.Columns.Clear();
                    calc.Columns.Add("total", "إجمالي الإيداعات");
                    calc.Columns.Add("total_minus", "إجمالي السحب");
                    calc.Columns.Add("status", "حالة حساب البنك الحالي");
                    ...
                    for (int i = 0; i < tableB.Rows.Count; i++)
                    {
                        if (tableB[6, i].Value.ToString() == "توريد")
                        {
                            getTotal_add = getTotal_add + Convert.ToDouble(tableB[3, i].Value.ToString());
                        }
                        else if (tableB[6, i].Value.ToString() == "صرف")
                        {
                            getTotal_remove = getTotal_remove + Convert.ToDouble(tableB[3, i].Value.ToString());

                        }
                    }
                    getTotal_Bank = Convert.ToDouble(tableA[3, 0].Value.ToString());    // get balance of bank

                    calc.Rows.Add(getTotal_add, getTotal_remove, getTotal_Bank);
        ...
        private void PrinterSelection_SelectedIndexChanged(object sender, EventArgs e)
        {
            ...
            if (PrinterSelection.SelectedIndex == 0)
            {
                ...
                else
                {
                    ...
                    printer.SendPRinter("بيان بالشيكات", tableA);
                }
            }
            else
            {
        ...
        private void Cheueque_Reports_KeyDown(object sender, KeyEventArgs e)
        {
        ...
        private void Cheueque_Reports_FormClosed(object sender, FormClosedEventArgs e)
        {
            this.Dispose();
