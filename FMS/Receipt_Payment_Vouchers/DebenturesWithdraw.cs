...
{
    public partial class DebenturesWithdraw : Form
    {
        DataTable dt = new DataTable();
        DataTable dt3 = new DataTable();
        double GetTreBalance = 0.0;

        public DebenturesWithdraw()
        {
        ...
        private void backgroundWorker_DoWork(object sender, System.ComponentModel.DoWorkEventArgs e)
        {
            ...
            SQLSERVER_Queries.PaymentVoucher_Scripts.DebenturesCashing debentures = new SQLSERVER_Queries.PaymentVoucher_Scripts.DebenturesCashing();
        ...
        private void tre_balance_TextChanged(object sender, EventArgs e)
        {
            if (tre_balance.Text != "")
            {
                GetTreBalance = Convert.ToDouble(tre_balance.Text);
            }
        }

        private void cheuque_adding_Click(object sender, EventArgs e)
        {
            double GetTreAmount = Convert.ToDouble(amount.Text);
            ClickSound sound = new ClickSound();
            sound.RunClickSound();
            if (Treasury_Name.SelectedIndex == -1)
            {
            ...
            else if (GetTreBalance <= 0)
            {
                MessageBox.Show("رصيد الخزينة الحالي لا يسمح \n برجاء ايداع رصيد", "ERROR", MessageBoxButtons.OK, MessageBoxIcon.Error);

            }
            else if (GetTreAmount > GetTreBalance)
            {
                MessageBox.Show("برجاء ادخال مبلغ اقل من رصيد الخزينة", "ERROR", MessageBoxButtons.OK, MessageBoxIcon.Error);
        ...
        private void DebenturesWithdraw_FormClosed(object sender, FormClosedEventArgs e)
        {
            this.Dispose();
