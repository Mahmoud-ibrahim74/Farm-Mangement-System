    ...
    class DebenturesAdding
    {
        ...
        public void DisplayData(DataTable dt, DataTable dt3, Guna2ComboBox Treasury_Name)
        {
            ...
            try
            {
                ...
                cmd.CommandText = "select prem_id as [رقم الإذن], date as[التاريخ],Treasury as [الخزينة],acc_name as[اسم الحساب],amount as [المبلغ],acc_type [نوع الحساب],notes as [ملاحظات] from Premession_Cash where service_type = N'توريد' and  date = CAST(GETDATE() AS date);";
                cmd.ExecuteNonQuery();
        ...
        public void InsertData(Guna2DateTimePicker getDate, Guna2ComboBox Treasury_Name, Guna2ComboBox acc_name, Guna2TextBox amount, Guna2TextBox getAccount_Type, Guna2TextBox notes)
        {
...

            {
                SqlConnection cn = new SqlConnection(cloud.StartConnection);
                cn.Open();
                SqlCommand cmd = new SqlCommand("insert into Premession_Cash(date,Treasury,acc_name,amount,acc_type,notes,service_type)  Values (@dat,@t,@an,@am,@at,@not,N'توريد')", cn);
