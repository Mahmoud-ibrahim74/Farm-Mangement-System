...
{
    public partial class goodsPurshase : Form
    {
        DataTable dt = new DataTable();
        DataTable dt3 = new DataTable();

        public goodsPurshase()
        {
            InitializeComponent();
        ...
        private void backgroundWorker_DoWork(object sender, System.ComponentModel.DoWorkEventArgs e)
        {
            ...
            SQLSERVER_Queries.workingScripts.ProductPurshaseScript productPurshase = new SQLSERVER_Queries.workingScripts.ProductPurshaseScript();
            ...
            if (ControlName == "displayData")
            {
                dt.Clear();
                dt3.Clear();
                productPurshase.DisplayData(storage, dt, dt3);
                productPurshase.getAmountGoods(amount, price, storage, product_name);
            ...
            {
                productPurshase.StorageRetreive(product_name, storage);
            }
            ...
            {
                productPurshase.InsertPremession(getDate, acc_type, anavar_name, storage, payment,treasury, product_name, unit, amount, price, total, notes);
                productPurshase.getAmountGoods(amount, price, storage, product_name);
            }         
            else if (ControlName == "deleteCols")
            {
                productPurshase.deleteColumns(tableB,product_name);
                productPurshase.getAmountGoods(amount, price, storage, product_name);
            }
          
            else if (ControlName == "saveBill")
            {
                productPurshase.SaveBill(product_name,product_value,transmit_value,added_tax,bill_value);
            }
            
            else
            {
                productPurshase.LoadData(treasury, storage, anavar_name, acc_type);
            }
        ...
        private void guna2Button4_Click(object sender, EventArgs e)
        {
            ...
            else if (acc_type.SelectedIndex == -1)
            {
                errorProvider.SetError(acc_type, "اختر نوع الحساب ");
            }
            ...
            else if (treasury.SelectedIndex == -1)
            {
                errorProvider.SetError(treasury, "اختر الخزينة ");
        ...
        private void goodsPurshase_KeyDown(object sender, KeyEventArgs e)
        {
        ...
        private void goodsPurshase_FormClosed(object sender, FormClosedEventArgs e)
        {
            this.Dispose();
