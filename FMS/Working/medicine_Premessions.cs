...
{
    public partial class medicine_Premessions : Form
    {
        DataTable dt = new DataTable();
        DataTable dt3 = new DataTable();

        public medicine_Premessions()
        {
            InitializeComponent();
        ...
        private void backgroundWorker_DoWork(object sender, System.ComponentModel.DoWorkEventArgs e)
        {
            ...
            SQLSERVER_Queries.workingScripts.medcine_Working _medcine = new SQLSERVER_Queries.workingScripts.medcine_Working();
            ...
            if (ControlName == "displayData")
            {
                dt.Clear();
                dt3.Clear();
                _medcine.DisplayData(medicine, dt, dt3);
                _medcine.getMedecineLoad(medicine, numbers, unit,cost,total);
            ...
            {
                _medcine.getAnavarAnimals(animals_type, anavar);
            ...
            {
                _medcine.InsertPremession(getDate, medicine, storage, anavar, desease, animals_type, numbers, unit, cost, total);
                _medcine.getMedecineLoad(medicine, numbers, unit, cost, total);
            }        
            else if (ControlName == "deleteCols")
            {
                _medcine.deleteColumns(tableB,medicine);
                _medcine.getMedecineLoad(medicine, numbers, unit, cost, total);
            }
            else if (ControlName == "saveBill")
            {
                _medcine.SaveBill(medicine, product_value, transmit_value, added_tax, bill_value);
            }

            else
            {
                _medcine.LoadData(medicine, anavar);
            }
        ...
        private void medicine_Premessions_KeyDown(object sender, KeyEventArgs e)
        {
        ...
        private void medicine_Premessions_FormClosed(object sender, FormClosedEventArgs e)
        {
            this.Dispose();
