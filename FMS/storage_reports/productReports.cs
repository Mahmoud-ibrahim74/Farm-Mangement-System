...
" *- Press (Merge Button)  to merge two or more PDF files in one PDF file.");
        ...
        private void guna2Button3_Click(object sender, System.EventArgs e)
        {
            ...
            sound.RunClickSound();
            if (getStorage.SelectedIndex == -1 || service_type.SelectedIndex == -1)
            {
        ...
        private void backgroundWorker_DoWork(object sender, System.ComponentModel.DoWorkEventArgs e)
        {
            ...
            SQLSERVER_Queries.SotrageReports.StorageScript storage = new SQLSERVER_Queries.SotrageReports.StorageScript();
            ...
            if (ControlName == "displayData")
            {
                dt.Clear();
                dt3.Clear();
                storage.DisplayData(service_type, product_type, getStorage, dt, dt3);
                tableB.DataSource = dt;
                tableA.DataSource = dt3;
            }
            else if (ControlName == "getGoods")
            {
                storage.getGoodsStorage(product_type, getStorage);
            }
            else if (ControlName == "displayByInterval")
            {
                dt.Clear();
                dt3.Clear();
                storage.DisplayByInterval(service_type, product_type, getStorage, fromDate, toDate, dt, dt3);
                tableB.DataSource = dt;
                tableA.DataSource = dt3;
            }
            else
        ...
        private void guna2GradientButton1_Click(object sender, EventArgs e)
        {
            ...
            else
            {
                ...
                if (service_type.SelectedIndex == 0)  // شراء
                {
                    ...
                    calc.Columns.Add("total", "اجمالي الكمية لعمليات الشراء (كيلو)");
                    calc.Columns.Add("afterAdded", "إجمالي التكلفة الكلية لعمليات الشراء (كيلو)");

                    for (int i = 0; i < tableB.RowCount; i++)
                    {
                        if (tableB[7, i].Value.ToString() == "طن")
                        {
                            getTotal = getTotal + (Convert.ToDouble(tableB[8, i].Value.ToString()) * 1000);
                ...
                else if (service_type.SelectedIndex == 1) // صرف
                {
                    ...
                    calc.Columns.Add("total", "اجمالي الكمية لمعليات الصرف");
                    calc.Columns.Add("afterAdded", "إجمالي التكلفة الكلية لعمليات الصرف");
                ...
                else
                {
                    ...
                    calc.Columns.Add("total_minus", "إجمالي كمية البضاعة المنصرفة للعنابر");
                    ...
                    for (int i = 0; i < tableB.RowCount; i++)
                    {
                        if (tableB[12, i].Value.ToString() == "شراء")
                        {
                            if (tableB[8, i].Value.ToString() == "طن")
                            {
                                getTotal_add_am = getTotal_add_am + (Convert.ToDouble(tableB[9, i].Value.ToString()) * 1000);
                                getTotal_add_cost = getTotal_add_cost + Convert.ToDouble(tableB[11, i].Value.ToString());
                            }
                            else
                            {
                                getTotal_add_am = getTotal_add_am + Convert.ToDouble(tableB[9, i].Value.ToString());
                                getTotal_add_cost = getTotal_add_cost + Convert.ToDouble(tableB[11, i].Value.ToString());
                            }

                        }
                        else if (tableB[12, i].Value.ToString() == "صرف")
                        {
                            if (tableB[8, i].Value.ToString() == "طن")
                            {
                                getTotal_remove_am = getTotal_remove_am + (Convert.ToDouble(tableB[9, i].Value.ToString()) * 1000);
                                getTotal_remove_cost = getTotal_remove_cost + Convert.ToDouble(tableB[11, i].Value.ToString());
                            }
                            else
                            {
                                getTotal_remove_am = getTotal_remove_am + Convert.ToDouble(tableB[9, i].Value.ToString());
                                getTotal_remove_cost = getTotal_remove_cost + Convert.ToDouble(tableB[11, i].Value.ToString());
                            }
                        }
                    }
                    
                    for (int i = 0; i < tableA.RowCount; i++)
                    {
                        getTotal_Storage = getTotal_Storage + Convert.ToDouble(tableA[3, i].Value.ToString());  // get amount  of storage
                        getTotal_Storage_cost = getTotal_Storage_cost + Convert.ToDouble(tableA[5, i].Value.ToString());  // get cost  of storage
                    }


                    calc.Rows.Add(getTotal_add_am, getTotal_remove_am, ((getTotal_Storage + getTotal_add_am) - getTotal_remove_am), (getTotal_Storage_cost + getTotal_add_cost) - getTotal_remove_cost);
        ...
        private void PrinterSelection_SelectedIndexChanged(object sender, EventArgs e)
        {
            ...
            if (PrinterSelection.SelectedIndex == 0)
            {
                ...
                else
                {
                    ...
                    PrinterSender printer = new PrinterSender();
                    printer.SendPRinter("بيان بالبضاعة", tableA);
                }
            }
            else
            {
                ...
                else
                {
                    ...
                    PrinterSender printer = new PrinterSender();
                    printer.SendPRinter("بيان بالبضاعة", tableB);
                }
            }
        }
        ...
        private void search_TextChanged(object sender, EventArgs e)
        {
            if (tableB.DataSource != null)
            {
                ...
                String search_value = "CONVERT(الكمية, System.String) LIKE '%{0}%'";
                (tableB.DataSource as DataTable).DefaultView.RowFilter = string.Format(search_value, search.Text);
            }
        }

        private void productReports_FormClosed(object sender, FormClosedEventArgs e)
        {
            this.Dispose();
